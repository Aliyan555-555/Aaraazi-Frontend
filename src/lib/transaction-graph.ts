...
function buildGraphFromDeal(dealId: string): TransactionGraph {
  const graph: TransactionGraph = {};
  const deals = getDeals(undefined, undefined);
  ...
  if (deal.cycles.sellCycle) {
    const sellCycles = getSellCycles(undefined, undefined);
    const sellCycle = sellCycles.find(
      (sc) => sc.id === deal.cycles.sellCycle?.id,
    );
    if (sellCycle) {
      graph.sellCycle = sellCycle;

      // Get property from sell cycle
      const properties = getProperties(undefined, undefined);
  ...
  if (deal.cycles.purchaseCycle) {
    const purchaseCycles = getPurchaseCycles(undefined, undefined);
    const purchaseCycle = purchaseCycles.find(
      (pc) => pc.id === deal.cycles.purchaseCycle?.id,
    );
    if (purchaseCycle) {
      ...
      if (!graph.property) {
        ...
        const property = properties.find(
          (p) => p.id === purchaseCycle.propertyId,
        );
        if (property) {
          graph.property = property;
        }
      ...
      if (purchaseCycle.buyerRequirementId) {
        ...
        const buyerRequirement = buyerRequirements.find(
          (br) => br.id === purchaseCycle.buyerRequirementId,
        );
        if (buyerRequirement) {
...
function buildGraphFromSellCycle(sellCycleId: string): TransactionGraph {
  ...
  if (
    sellCycle.winningPurchaseCycleId &&
    sellCycle.winningPurchaseCycleId !== graph.purchaseCycle?.id
  ) {
    const purchaseCycles = getPurchaseCycles(undefined, undefined);
    const winningPurchaseCycle = purchaseCycles.find(
      (pc) => pc.id === sellCycle.winningPurchaseCycleId,
    );
 ...
 */
function buildGraphFromPurchaseCycle(
  purchaseCycleId: string,
): TransactionGraph {
  const graph: TransactionGraph = {};
  const purchaseCycles = getPurchaseCycles(undefined, undefined);
  ...
  if (purchaseCycle.linkedSellCycleId) {
    const sellCycles = getSellCycles(undefined, undefined);
    const sellCycle = sellCycles.find(
      (sc) => sc.id === purchaseCycle.linkedSellCycleId,
    );
    if (sellCycle) {
      graph.sellCycle = sellCycle;
    }
...
function buildGraphFromProperty(propertyId: string): TransactionGraph {
  ...
  if (property.activeSellCycleIds && property.activeSellCycleIds.length > 0) {
    const sellCycles = getSellCycles(undefined, undefined);
    const sellCycle = sellCycles.find(
      (sc) => sc.id === property.activeSellCycleIds![0],
    );
    if (sellCycle) {
      graph.sellCycle = sellCycle;
  ...
  if (
    property.activePurchaseCycleIds &&
    property.activePurchaseCycleIds.length > 0
  ) {
    const purchaseCycles = getPurchaseCycles(undefined, undefined);
    const purchaseCycle = purchaseCycles.find(
      (pc) => pc.id === property.activePurchaseCycleIds![0],
    );
    if (purchaseCycle) {
 ...
 */
function buildGraphFromBuyerRequirement(
  buyerRequirementId: string,
): TransactionGraph {
  const graph: TransactionGraph = {};
  const buyerRequirements = getBuyerRequirements();
  const buyerRequirement = buyerRequirements.find(
    (br) => br.id === buyerRequirementId,
  );
  ...
  const purchaseCycles = getPurchaseCycles(undefined, undefined);
  const purchaseCycle = purchaseCycles.find(
    (pc) => pc.buyerRequirementId === buyerRequirementId,
  );
...
export function getUnifiedTimeline(dealId: string): TransactionTimeline[] {
  ...
  if (graph.property) {
    const propertyName =
      graph.property.title || formatPropertyAddress(graph.property.address);
    timeline.push({
  ...
  if (graph.deal) {
    ...
    // Deal completion
    if ((graph.deal.metadata as any).completedAt) {
      timeline.push({
        date: (graph.deal.metadata as any).completedAt,
...
export function areEntitiesRelated(
  ...
  if (graph1.property?.id && graph1.property.id === graph2.property?.id)
    return true;
  if (graph1.sellCycle?.id && graph1.sellCycle.id === graph2.sellCycle?.id)
    return true;
  if (
    graph1.purchaseCycle?.id &&
    graph1.purchaseCycle.id === graph2.purchaseCycle?.id
  )
    return true;
  if (graph1.deal?.id && graph1.deal.id === graph2.deal?.id) return true;
  if (
    graph1.buyerRequirement?.id &&
    graph1.buyerRequirement.id === graph2.buyerRequirement?.id
  )
    return true;
...
export function getPropertyDeals(propertyId: string): Deal[] {
  const property = getProperties(undefined, undefined).find(
    (p) => p.id === propertyId,
  );
  if (!property) return [];
  ...
  return deals.filter((deal) => {
    ...
    if (
      deal.cycles.sellCycle &&
      deal.cycles.sellCycle.propertyId === propertyId
    ) {
      return true;
    }
...
export function getPropertyCycles(propertyId: string) {
  const sellCycles = getSellCycles(undefined, undefined).filter(
    (sc) => sc.propertyId === propertyId,
  );
  const purchaseCycles = getPurchaseCycles(undefined, undefined).filter(
    (pc) => pc.propertyId === propertyId,
  );

  return {
    sellCycles,
    purchaseCycles,
    all: [...sellCycles, ...purchaseCycles].sort(
      (a, b) =>
