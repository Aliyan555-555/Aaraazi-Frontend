 ...
 */
export const getCustomReportById = (
  id: string,
...
export const updateCustomReport = (
  id: string,
  updates: Partial<CustomReportTemplate>,
): void => {
  try {
    const templates = getCustomReports();
...
export const generateReport = (
  ...
  return {
    ...
    data: {
      rows: sortedData,
      summary: {}, // Placeholder for summary stats if needed
...
const selectFields = (data: any[], fields: SelectedField[]): ReportRow[] => {
  return data.map((row) => {
    const newRow: ReportRow = {
      id: row.id || row.deal_id || row.property_id || `row_${Math.random()}`,
    };
...
const applyGrouping = (
  ...
  Object.entries(groups).forEach(([groupKey, groupRows]) => {
    const newRow: ReportRow = { id: groupKey };
