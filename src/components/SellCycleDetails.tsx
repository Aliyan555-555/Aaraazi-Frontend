...
// Remove stubs - using real APIs now
...
export function SellCycleDetails({
    ...
    try {
      ...
      const message =
        error && typeof error === 'object' && 'message' in error
          ...
          : `Failed to ${confirmAction.type} offer`;
      toast.error(message);
    } finally {
  ...
  // Update cycle when initialCycle changes (from refetch)
  useEffect(() => {
  ...
  // Get linked deal - for now, we just check if linkedDealId exists
  // Full deal fetching can be added later via useDeal hook
  const linkedDeal = useMemo(() => {
    return cycle.linkedDealId ? { id: cycle.linkedDealId } as any : null;
  }, [cycle.linkedDealId]);
      ...
      <MetricCardsGroup
        metrics={[
          ...
          },
          {
            label: 'Rejected',
            value: (
              cycle.offers?.filter((o) => o.status === OFFER_STATUS.REJECTED).length || 0
            ).toString(),
        ...
        <div className="fixed bottom-6 right-6 z-50">
          <ShareToggle
            ...
            onToggle={(isShared, userId, userName) => {
              // TODO: Implement sharing toggle API
              toast.info('Sharing toggle coming soon');
              onUpdate();
            }}
          />
        </div>
