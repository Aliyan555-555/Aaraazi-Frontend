...
import type { PurchaseCycleApiResponse } from '@/lib/api/purchase-cycles';

function mapApiToPurchaseCycle(api: PurchaseCycleApiResponse): PurchaseCycle {
  ...
  return {
    ...
    title: api.propertyListing?.title || `Purchase ${api.cycleNumber}`,
...
export interface PurchaseCyclesWorkspaceProps {
  user: User;
...
/**
 * PurchaseCyclesWorkspace - Complete workspace using template system
 */
export const PurchaseCyclesWorkspace: React.FC<PurchaseCyclesWorkspaceProps> = ({
  user,
  onNavigate,
  onStartNew,
  onEditCycle,
  ...
  const handleFilter = useCallback((cycle: PurchaseCycle, activeFilters: Map<string, any>): boolean => {
    ...
    if (Array.isArray(financingTypeFilter) && financingTypeFilter.length > 0) {
      const financingType = (cycle as any).financingType;
      if (!financingType || !(financingTypeFilter as string[]).includes(financingType)) return false;
    }
